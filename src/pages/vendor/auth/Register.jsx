...
const Register = () => {
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (allValues.profileImg)
        formData.append("profileImg", getFile(allValues.profileImg));
      ...
      if (allValues.gstImage)
        formData.append("gstImage", getFile(allValues.gstImage));
      if (allValues.foodImage)
        formData.append("foodImage", getFile(allValues.foodImage));
  ...
  const steps = [
    ...
    {
      title: "Agreement",
      content: <Agreement termCondition={termCondition} fee={fee} />,
    },
  ];
  ...
  return (
    <>
      <title>Findin | Vendor Registration</title>
      <div className="flex min-h-screen bg-gray-100">
        ...
        <div className="flex w-full lg:w-1/2 justify-center items-center px-4 sm:px-6 py-4">
          <div className="w-full max-w-3xl bg-white shadow-2xl rounded-2xl p-2 sm:p-10">
            ...
            <p className="text-sm text-gray-500 mb-8 my-4">
              Already registred{" "}
              <Link to={"/vendor/login"} className="text-blue-500">
                Login
              </Link>
              !
            </p>
